...
void loop()
{
  while (1)
  {
    ...
    while (!OrangutanPushbuttons::isPressed(BUTTON_B) || mode!=3)
    {
      //----------------模式選擇----------------
    ...
    if(mode==1)//慢速正常
    {
      for (i = 0; i < path_length; i++)
      {
        startTime=millis();
        follow_segment();
        T[i]=(millis()-startTime)*AtoB-as;
        run_turn(path[i]);
      }
      startTime=millis();
      follow_segment();
      T[41]=(millis()-startTime)*AtoB-as;
    }
    else if(mode==2)//直接衝刺
    {
      for (i = 0; i < path_length; i++)
